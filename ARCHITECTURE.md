...
**Graph optimization** (`src/graph_opt.rs`): After graph-source selection and before emission, `DelimitedParts` are rebuilt from the structural output, terminator stream, and optional nesting streams. The pass prunes unreachable nodes, CSEs identical ops, canonicalizes commutative bitwise forms, and folds local boolean identities under the same AVX2 cost model used by synthesis. Stateful ops (`ShiftLeft1`, `PrefixXor`, `Add`, `Regions`) stay opaque except for optimized operands and duplicate-node merging, so output-role remapping is explicit and testable. Codegen accepts a rebuilt graph only when its AVX2 cost is lower than the source graph; equal-cost rewrites remain available to tests but do not reorder emitted kernels by default.
