...
**Graph optimization** (`src/graph_opt.rs`): After graph-source selection and before emission, `DelimitedParts` are rebuilt from the structural output, terminator stream, and optional nesting streams. Rewrites come in two tiers. The conservative tier never grows the graph: unreachable-node sweeping, CSE, commutative canonicalization, constant folding, and local boolean identities. The speculative tier restructures: Not-extraction through Xor (`a ^ !b → !(a ^ b)`, which lets an enclosing Not cancel — this collapses the odd-backslash escape chain `!(EVEN ^ !Add(..)) → EVEN ^ Add(..)` in every backslash-escape dialect, two ops shorter per block) and class-algebra fusion (`And`/`Or`/`Xor` over two `Class` operands folds into one class node covering the combined membership, capped at 8 members so fusion never leaves codegen's compare-friendly range). Speculative rewrites can lose when the rewritten subterm is shared, so both candidates are built and the cheaper under the AVX2 cost model is extracted — the build-candidates-then-extract shape a future e-graph would scale up. Stateful ops (`ShiftLeft1`, `PrefixXor`, `Add`, `Regions`) stay opaque except for optimized operands and duplicate-node merging, so output-role remapping is explicit and testable. Codegen accepts a rebuilt graph only when its AVX2 cost is strictly lower than the source graph. The pass fires on real formats — it takes 2 bitwise ops per block out of the logfmt/NDJSON/JSON kernels (including out of the weighted synthesizer's own output, whose per-stream search cannot see the cross-stream Not cancellation) and fuses csv_hash's structural `Or` into one class. Emission order matters as much as op count: codegen emits nodes in graph order, and a naive left-to-right rebuild hoisted leaf classes to the top of the block, extending their live ranges across the whole computation — interleaved A/B benchmarks showed a consistent 5–10% regression from the extra register pressure even though the graph was smaller. The rebuilder therefore visits the deeper operand subtree first (Sethi–Ullman order), which places each class load adjacent to its consumer. Measured honestly with interleaved A/B runs against unchanged-kernel controls: LLVM already folds the boolean identities itself, so the net throughput effect of the smaller graphs is parity (slightly ahead on logfmt/NDJSON, within noise) — the durable wins are minimal emitted source, shorter live ranges, and the same simplifications for every backend less aggressive than LLVM (the interpreter and byte-serial prover paths). `cargo run --release --example opt_probe` reports per-format stats for both graph sources.
