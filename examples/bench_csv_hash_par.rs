...
fn main() {
    ...
    // Headline: interleave serial vs par@24 (sweet spot) so they share run
    // conditions — the trustworthy A/B per this box's variance.
    let (mut serial, mut par24) = (Vec::new(), Vec::new());
    for _ in 0..15 {
        let t = Instant::now();
