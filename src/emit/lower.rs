...
pub fn index_function(graph: &Graph, name: &str) -> Item {
    ...
    // Full 64-byte blocks.
    let mut loop_body = block_body(graph, &live, out_id, false);
    ...
    body.push(Stmt::If {
        ...
        then: Block(block_body(graph, &live, out_id, true)),
        els: None,
    });
    ...
    Item::from(
        Func::new(name)
            ...
            .doc("graph with an AVX2/scalar `class_mask`; byte-equivalent to")
...
pub fn lower_index_c(dialect: &Dialect) -> Vec<Item> {
    ...
    items.push(index_function_c(&graph, "index_structurals"));
    items
}
...
fn index_function_c(graph: &Graph, name: &str) -> Item {
    let live = live_nodes(graph);
    let out_id = graph.output().0 as usize;
    let mut body: Vec<Stmt> = Vec::new();
    for (i, op) in graph.nodes().iter().enumerate() {
    ...
    let mut loop_body = block_body_c(graph, &live, out_id, false);
    loop_body.push(Stmt::assign_op(
        Expr::path("offset"),
        BinOp::Add,
        Expr::int(64),
    ...
    body.push(Stmt::let_(
        ...
        false,
        Type::name("usize"),
        Expr::binary(BinOp::Sub, Expr::path("len"), Expr::path("offset")),
    ));
    body.push(Stmt::If {
        cond: Expr::binary(BinOp::Gt, Expr::path("rem"), Expr::int(0)),
        then: Block(block_body_c(graph, &live, out_id, true)),
        els: None,
    });
    Item::from(
        Func::new(name)
...
fn block_body_c(graph: &Graph, live: &[bool], out_id: usize, tail: bool) -> Vec<Stmt> {
    let mut b = Vec::new();
    if tail {
        b.push(Stmt::Raw("uint8_t tail[64] = {0};".into()));
        b.push(Stmt::Raw(
...
/// Per-node statements for the C backend: same algebra as [`node_stmts`] but
/// every `let` is `uint64_t`, `Class` uses a C compound literal, and `Add` is a
/// plain `+` (unsigned C arithmetic wraps).
fn node_stmts_c(i: usize, op: &Op) -> Vec<Stmt> {
    let n = format!("n{i}");
    let nref = |id: crate::ir::NodeId| Expr::path(format!("n{}", id.0));
    let let64 = |name: String, init: Expr| Stmt::Let {
        name,
        mutable: false,
        ty: Some(Type::name("u64")),
        init,
    };
    match *op {
        Op::Class(class) => {
            let members: Vec<String> = class.members().map(|b| b.to_string()).collect();
            let init = Expr::raw(format!(
                "class_mask(block, (const uint8_t[]){{{}}}, {})",
                members.join(", "),
                members.len()
            ));
            vec![let64(n, init)]
        }
        Op::Const(pattern) => vec![let64(n, Expr::hex(pattern))],
        Op::Not(a) => vec![let64(n, Expr::unary(UnOp::Not, nref(a)))],
        Op::And(a, b) => vec![let64(n, Expr::binary(BinOp::BitAnd, nref(a), nref(b)))],
        Op::Or(a, b) => vec![let64(n, Expr::binary(BinOp::BitOr, nref(a), nref(b)))],
        Op::Xor(a, b) => vec![let64(n, Expr::binary(BinOp::BitXor, nref(a), nref(b)))],
        ...
        Op::Add(a, b) => {
            ...
            let c2 = format!("c2_{i}");
            vec![
                let64(sum.clone(), Expr::binary(BinOp::Add, nref(a), nref(b))),
                let64(
                    c1.clone(),
                    Expr::cast(
        ...
        Op::Regions(q, s, t) => {
            ...
            vec![let64(
                ...
                Expr::call(
                    ...
                    vec![nref(q), nref(s), nref(t), Expr::raw(format!("&{carry}"))],
                ),
            )]
        }
    }
...
fn block_body(graph: &Graph, live: &[bool], out_id: usize, tail: bool) -> Vec<Stmt> {
    let mut b = Vec::new();
    if tail {
        b.push(Stmt::Raw("let mut tail = [0u8; 64];".into()));
        b.push(Stmt::Raw(
            "tail[..rem].copy_from_slice(&data[offset..]);".into(),
        ));
        b.push(Stmt::let_(
            "block",
            false,
            Type::slice(Type::name("u8")),
            Expr::raw("&tail"),
        ));
    } else {
        b.push(Stmt::let_(
    ...
    let out_node = Expr::path(format!("n{out_id}"));
    let mask_init = if tail {
        Expr::binary(BinOp::BitAnd, out_node, Expr::raw("((1u64 << rem) - 1)"))
    } else {
        out_node
    };
    b.push(Stmt::let_("mask", true, Type::name("u64"), mask_init));
...
fn node_stmts(i: usize, op: &Op) -> Vec<Stmt> {
    let n = format!("n{i}");
    let nref = |id: crate::ir::NodeId| Expr::path(format!("n{}", id.0));
    let inferred = |name: String, init: Expr| Stmt::Let {
        name,
        mutable: false,
        ty: None,
        init,
    ...
    match *op {
        Op::Class(class) => {
            ...
            vec![inferred(
                ...
                Expr::call(
                    ...
                    vec![
                        Expr::path("block"),
                        Expr::raw(format!("&[{}]", members.join(", "))),
                    ],
                ),
            )]
        }
        Op::Const(pattern) => vec![Stmt::let_(n, false, Type::name("u64"), Expr::hex(pattern))],
        Op::Not(a) => vec![inferred(n, Expr::unary(UnOp::Not, nref(a)))],
        Op::And(a, b) => vec![inferred(n, Expr::binary(BinOp::BitAnd, nref(a), nref(b)))],
        Op::Or(a, b) => vec![inferred(n, Expr::binary(BinOp::BitOr, nref(a), nref(b)))],
        Op::Xor(a, b) => vec![inferred(n, Expr::binary(BinOp::BitXor, nref(a), nref(b)))],
        Op::ShiftLeft1(a) | Op::ShiftLeft1Seeded(a) => {
            let carry = format!("carry_{i}");
            vec![
                inferred(
        ...
        Op::Add(a, b) => {
            ...
            let c2 = format!("c2_{i}");
            vec![
                inferred(sum.clone(), Expr::binary(BinOp::WrapAdd, nref(a), nref(b))),
                inferred(
                    c1.clone(),
                    Expr::cast(
                        Expr::binary(BinOp::Lt, Expr::path(sum.clone()), nref(a)),
