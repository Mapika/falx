...
pub fn rust_index_items(graph: &Graph, name: &str) -> Vec<Item> {
    ...
    let live = live_nodes(graph);
    vec![
        x86_dispatch(graph, name),
        baked_x86_variant(IsaInline::Avx512, graph, &live, name),
        baked_x86_variant(IsaInline::Avx2, graph, &live, name),
        emit_index(Target::Rust, graph, &format!("{name}_portable")),
    ]
}

/// The `pub fn <name>` dispatcher: pick the best x86 variant at runtime, else
/// the portable fallback.
fn x86_dispatch(graph: &Graph, name: &str) -> Item {
    ...
    Item::Raw(format!(
        r#"/// Structural indexer: dispatches to a baked AVX-512/AVX2 whole-loop variant
/// on x86, else the portable `class_mask` path. Generated by `emit::lower`.
...
fn baked_x86_variant(isa: IsaInline, graph: &Graph, live: &[bool], name: &str) -> Item {
    use std::fmt::Write;
    let out_id = graph.output().0;
    let has_px = graph.nodes().iter().any(|o| matches!(o, Op::PrefixXor(_)));
    let (suffix, feats) = match isa {
        IsaInline::Avx512 if has_px => ("avx512", "avx512f,avx512bw,pclmulqdq"),
        IsaInline::Avx512 => ("avx512", "avx512f,avx512bw"),
        IsaInline::Avx2 if has_px => ("avx2", "avx2,pclmulqdq"),
        IsaInline::Avx2 => ("avx2", "avx2"),
    };
    let ones = if has_px {
        "    let ones = _mm_set1_epi8(-1);\n"
    } else {
...
fn baked_node_line(isa: IsaInline, i: usize, op: &Op) -> String {
    ...
    match *op {
        ...
        Op::PrefixXor(a) => format!(
            "        let n{i} = (_mm_cvtsi128_si64(_mm_clmulepi64_si128(_mm_set_epi64x(0, {a} as i64), ones, 0)) as u64) ^ carry_{i};\n        carry_{i} = ((n{i} as i64) >> 63) as u64;\n",
            a = nr(a)
        ),
        Op::Add(a, b) => format!(
