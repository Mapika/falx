// Generated by falx from format `csv_geo`. Do not edit by hand;
...
fn index_csv_geo_stats_dispatch(data: &[u8], seed: u64, start: usize, sink: &mut CsvGeoStatsSink) {
    ...
    {
        // SAFETY: the required target features were just detected.
        unsafe { avx512::index_csv_geo_stats_compress(data, seed, start, sink) };
        return;
    }
    #[cfg(target_arch = "x86_64")]
    if std::arch::is_x86_feature_detected!("avx512f")
    ...
    {
        // SAFETY: the required target features were just detected.
        unsafe { neon::index_csv_geo_stats(data, seed, start, sink) };
        return;
    }
    unsupported_cpu();
}
...
mod avx512 {
    ...
    /// [`index_csv_geo_stats`] twin for AVX-512 VBMI2 + BMI2 hosts: block structural
    /// positions arrive via one `vpcompressb` over a byte iota and terminator
    /// flags via `pext`, replacing the serial trailing-zero/clear-bit walk
    /// with a counted loop of independent loads. Scans 64-byte blocks from
    /// `start` (block-aligned) onward, until end of data or until the
    /// sink completes its record range.
    #[target_feature(enable = "avx512f", enable = "avx512bw", enable = "avx512vl", enable = "avx512vbmi2", enable = "bmi2", enable = "pclmulqdq")]
    pub(crate) fn index_csv_geo_stats_compress(data: &[u8], seed: u64, start: usize, sink: &mut super::CsvGeoStatsSink) {
        let mut carries = [seed];
        let mut offset = start;
        let iota = byte_iota();
        while offset + 64 <= data.len() {
