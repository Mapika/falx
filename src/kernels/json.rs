...
fn nested_tape(data: &[u8], tape: &mut Vec<u64>, stack: &mut Vec<u64>) -> Option<NestError> {
    ...
    {
        // SAFETY: the required target features were just detected.
        return unsafe { neon::nested_tape(data, tape, stack) };
    }
    unsupported_cpu()
}
...
pub fn parse_nested_par_into<'a>(
    ...
    let bounds: Vec<usize> = (0..=threads)
        ...
        .collect();
    let mut entries: Vec<[u64; 2]> = Vec::with_capacity(threads);
...
unsafe fn nested_tape_seeded_dispatch(
        ...
        && std::arch::is_aarch64_feature_detected!("aes")
    {
        // SAFETY: features detected; slot contract forwarded to caller.
        return unsafe {
            neon::nested_tape_seeded(data, seed, pos_base, master, tape_base, stack, pending)
        };
    }
    unsupported_cpu()
}
...
fn nested_prepass_dispatch(
    ...
    {
        // SAFETY: the required target features were just detected.
        unsafe { neon::nested_prepass(data, bounds, entries, counts) };
        return;
    }
    unsupported_cpu();
}
...
mod avx512 {
    ...
    unsafe fn step(ptr: *const u8, carries: &mut [u64; 2]) -> (u64, u64) {
        ...
        let v8 = !v7;
        let v9 = eq_mask(v, 44u8) | eq_mask(v, 58u8) | eq_mask(v, 91u8) | eq_mask(v, 93u8) | eq_mask(v, 123u8) | eq_mask(v, 125u8); // class ",:[]{}"
        let v10 = v9 & v8;
    ...
    unsafe fn step_nested(ptr: *const u8, carries: &mut [u64; 2]) -> (u64, u64, u64) {
        ...
        let v9 = eq_mask(v, 44u8) | eq_mask(v, 58u8) | eq_mask(v, 91u8) | eq_mask(v, 93u8) | eq_mask(v, 123u8) | eq_mask(v, 125u8); // class ",:[]{}"
        let v10 = v9 & v8;
        let v12 = eq_mask(v, 91u8) | eq_mask(v, 123u8); // class "[{"
        let v13 = v8 & v12;
        let v14 = eq_mask(v, 93u8) | eq_mask(v, 125u8); // class "]}"
        let v15 = v8 & v14;
        (v10, v13, v15)
    }
...
mod neon {
    ...
    unsafe fn step(ptr: *const u8, carries: &mut [u64; 2]) -> (u64, u64) {
        ...
        let v7 = { let parity = prefix_xor(v6) ^ carries[1]; carries[1] = ((parity as i64) >> 63) as u64; parity };
        let v8 = !v7;
        let v9 = eq_mask(b0, b1, b2, b3, 44u8) | eq_mask(b0, b1, b2, b3, 58u8) | eq_mask(b0, b1, b2, b3, 91u8) | eq_mask(b0, b1, b2, b3, 93u8) | eq_mask(b0, b1, b2, b3, 123u8) | eq_mask(b0, b1, b2, b3, 125u8); // class ",:[]{}"
        let v10 = v9 & v8;
        let v11 = eq_mask(b0, b1, b2, b3, 10u8); // class "\n"
        (v10, v10 & v11)
    }
    ...
    unsafe fn step_nested(ptr: *const u8, carries: &mut [u64; 2]) -> (u64, u64, u64) {
        ...
        let v9 = eq_mask(b0, b1, b2, b3, 44u8) | eq_mask(b0, b1, b2, b3, 58u8) | eq_mask(b0, b1, b2, b3, 91u8) | eq_mask(b0, b1, b2, b3, 93u8) | eq_mask(b0, b1, b2, b3, 123u8) | eq_mask(b0, b1, b2, b3, 125u8); // class ",:[]{}"
        let v10 = v9 & v8;
        let v12 = eq_mask(b0, b1, b2, b3, 91u8) | eq_mask(b0, b1, b2, b3, 123u8); // class "[{"
        let v13 = v8 & v12;
        let v14 = eq_mask(b0, b1, b2, b3, 93u8) | eq_mask(b0, b1, b2, b3, 125u8); // class "]}"
        let v15 = v8 & v14;
        (v10, v13, v15)
    }
