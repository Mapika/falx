...
impl<'a> Columns<'a> {
    fn with_capacity(data: &'a [u8], rows: usize) -> Self {
        Columns {
            ...
            title_data: Vec::with_capacity(rows * 8),
...
pub fn parse_columns_par(data: &[u8], threads: usize) -> Columns<'_> {
    ...
    cols.id.reserve(rows_total);
    let title_total: usize = parts.iter().map(|p| p.title_data.len()).sum();
    assert!(
        title_total <= i32::MAX as usize,
        "string column 'title' exceeds the 2 GiB Arrow i32-offset limit"
    );
    cols.title_data.reserve(title_total);
    cols.title_offsets.reserve(rows_total);
    cols.value.reserve(rows_total);
    cols.label.reserve(rows_total);
    {
        let mut id_rest = &mut cols.id.spare_capacity_mut()[..rows_total];
        let mut id_slots: Vec<&mut [std::mem::MaybeUninit<i64>]> =
            Vec::with_capacity(parts.len());
        let mut title_data_rest =
            &mut cols.title_data.spare_capacity_mut()[..title_total];
        let mut title_data_slots: Vec<&mut [std::mem::MaybeUninit<u8>]> =
            Vec::with_capacity(parts.len());
        let mut title_offsets_rest =
            &mut cols.title_offsets.spare_capacity_mut()[..rows_total];
        let mut title_offsets_slots: Vec<&mut [std::mem::MaybeUninit<i32>]> =
            Vec::with_capacity(parts.len());
        let mut value_rest = &mut cols.value.spare_capacity_mut()[..rows_total];
        let mut value_slots: Vec<&mut [std::mem::MaybeUninit<f64>]> =
            Vec::with_capacity(parts.len());
        let mut label_rest = &mut cols.label.spare_capacity_mut()[..rows_total];
        let mut label_slots: Vec<&mut [std::mem::MaybeUninit<(u32, u32)>]> =
            Vec::with_capacity(parts.len());
        for part in &parts {
            assert_eq!(part.id.len(), part.rows, "one cell per row");
            let (head, tail) = id_rest.split_at_mut(part.rows);
            id_slots.push(head);
            id_rest = tail;
            assert_eq!(part.title_offsets.len(), part.rows + 1, "one offset per row");
            let (head, tail) = title_data_rest.split_at_mut(part.title_data.len());
            title_data_slots.push(head);
            title_data_rest = tail;
            let (head, tail) = title_offsets_rest.split_at_mut(part.rows);
            title_offsets_slots.push(head);
            title_offsets_rest = tail;
            assert_eq!(part.value.len(), part.rows, "one cell per row");
            let (head, tail) = value_rest.split_at_mut(part.rows);
            value_slots.push(head);
            value_rest = tail;
            assert_eq!(part.label.len(), part.rows, "one cell per row");
            let (head, tail) = label_rest.split_at_mut(part.rows);
            label_slots.push(head);
            label_rest = tail;
        }
        std::thread::scope(|s| {
            let mut id_slots = id_slots.into_iter();
            let mut title_data_slots = title_data_slots.into_iter();
            let mut title_offsets_slots = title_offsets_slots.into_iter();
            let mut title_base = 0usize;
            let mut value_slots = value_slots.into_iter();
            let mut label_slots = label_slots.into_iter();
            for part in &parts {
                let id_slot = id_slots.next().expect("slot per part");
                let title_data_slot = title_data_slots.next().expect("slot per part");
                let title_offsets_slot = title_offsets_slots.next().expect("slot per part");
                let title_rebase = title_base as i32;
                title_base += part.title_data.len();
                let value_slot = value_slots.next().expect("slot per part");
                let label_slot = label_slots.next().expect("slot per part");
                s.spawn(move || {
                    // SAFETY: disjoint slot of spare capacity, exact length.
                    unsafe {
                        std::ptr::copy_nonoverlapping(
                            part.id.as_ptr(),
                            id_slot.as_mut_ptr().cast::<i64>(),
                            part.id.len(),
                        );
                    }
                    ...
                    unsafe {
                        std::ptr::copy_nonoverlapping(
                            part.title_data.as_ptr(),
                            title_data_slot.as_mut_ptr().cast::<u8>(),
                            part.title_data.len(),
                        );
                    }
                    for (dst, &o) in title_offsets_slot.iter_mut().zip(&part.title_offsets[1..]) {
                        dst.write(o + title_rebase);
                    }
                    // SAFETY: disjoint slot of spare capacity, exact length.
                    unsafe {
                        std::ptr::copy_nonoverlapping(
                            part.value.as_ptr(),
                            value_slot.as_mut_ptr().cast::<f64>(),
                            part.value.len(),
                        );
                    }
                    // SAFETY: disjoint slot of spare capacity, exact length.
                    unsafe {
                        std::ptr::copy_nonoverlapping(
                            part.label.as_ptr(),
                            label_slot.as_mut_ptr().cast::<(u32, u32)>(),
                            part.label.len(),
                        );
                    }
                });
    ...
    unsafe {
        cols.id.set_len(rows_total);
        cols.title_data.set_len(title_total);
        cols.title_offsets.set_len(1 + rows_total);
        cols.value.set_len(rows_total);
        cols.label.set_len(rows_total);
    }
    // Validity bitmaps are rows/8 bytes; the serial bit-shift join
    // is noise next to the scattered column copies above.
    let mut rows_done = 0usize;
    for part in &parts {
        append_bitmap(&mut cols.id_valid, rows_done, &part.id_valid, part.rows);
        append_bitmap(&mut cols.title_valid, rows_done, &part.title_valid, part.rows);
        append_bitmap(&mut cols.value_valid, rows_done, &part.value_valid, part.rows);
        append_bitmap(&mut cols.label_valid, rows_done, &part.label_valid, part.rows);
        rows_done += part.rows;
    }
