...
impl<'a> Columns<'a> {
    fn with_capacity(data: &'a [u8], rows: usize) -> Self {
        Columns {
            ...
            city_data: Vec::with_capacity(rows * 8),
...
pub fn parse_columns_par(data: &[u8], threads: usize) -> Columns<'_> {
    ...
    let city_total: usize = parts.iter().map(|p| p.city_data.len()).sum();
    assert!(
        city_total <= i32::MAX as usize,
        "string column 'city' exceeds the 2 GiB Arrow i32-offset limit"
    );
    cols.city_data.reserve(city_total);
    cols.city_offsets.reserve(rows_total);
    cols.latitude.reserve(rows_total);
    cols.longitude.reserve(rows_total);
    {
        let mut city_data_rest =
            &mut cols.city_data.spare_capacity_mut()[..city_total];
        let mut city_data_slots: Vec<&mut [std::mem::MaybeUninit<u8>]> =
            Vec::with_capacity(parts.len());
        let mut city_offsets_rest =
            &mut cols.city_offsets.spare_capacity_mut()[..rows_total];
        let mut city_offsets_slots: Vec<&mut [std::mem::MaybeUninit<i32>]> =
            Vec::with_capacity(parts.len());
        let mut latitude_rest = &mut cols.latitude.spare_capacity_mut()[..rows_total];
        let mut latitude_slots: Vec<&mut [std::mem::MaybeUninit<f64>]> =
            Vec::with_capacity(parts.len());
        let mut longitude_rest = &mut cols.longitude.spare_capacity_mut()[..rows_total];
        let mut longitude_slots: Vec<&mut [std::mem::MaybeUninit<f64>]> =
            Vec::with_capacity(parts.len());
        for part in &parts {
            assert_eq!(part.city_offsets.len(), part.rows + 1, "one offset per row");
            let (head, tail) = city_data_rest.split_at_mut(part.city_data.len());
            city_data_slots.push(head);
            city_data_rest = tail;
            let (head, tail) = city_offsets_rest.split_at_mut(part.rows);
            city_offsets_slots.push(head);
            city_offsets_rest = tail;
            assert_eq!(part.latitude.len(), part.rows, "one cell per row");
            let (head, tail) = latitude_rest.split_at_mut(part.rows);
            latitude_slots.push(head);
            latitude_rest = tail;
            assert_eq!(part.longitude.len(), part.rows, "one cell per row");
            let (head, tail) = longitude_rest.split_at_mut(part.rows);
            longitude_slots.push(head);
            longitude_rest = tail;
        }
        std::thread::scope(|s| {
            let mut city_data_slots = city_data_slots.into_iter();
            let mut city_offsets_slots = city_offsets_slots.into_iter();
            let mut city_base = 0usize;
            let mut latitude_slots = latitude_slots.into_iter();
            let mut longitude_slots = longitude_slots.into_iter();
            for part in &parts {
                let city_data_slot = city_data_slots.next().expect("slot per part");
                let city_offsets_slot = city_offsets_slots.next().expect("slot per part");
                let city_rebase = city_base as i32;
                city_base += part.city_data.len();
                let latitude_slot = latitude_slots.next().expect("slot per part");
                let longitude_slot = longitude_slots.next().expect("slot per part");
                s.spawn(move || {
                    ...
                    unsafe {
                        std::ptr::copy_nonoverlapping(
                            part.city_data.as_ptr(),
                            city_data_slot.as_mut_ptr().cast::<u8>(),
                            part.city_data.len(),
                        );
                    }
                    for (dst, &o) in city_offsets_slot.iter_mut().zip(&part.city_offsets[1..]) {
                        dst.write(o + city_rebase);
                    }
                    // SAFETY: disjoint slot of spare capacity, exact length.
                    unsafe {
                        std::ptr::copy_nonoverlapping(
                            part.latitude.as_ptr(),
                            latitude_slot.as_mut_ptr().cast::<f64>(),
                            part.latitude.len(),
                        );
                    }
                    // SAFETY: disjoint slot of spare capacity, exact length.
                    unsafe {
                        std::ptr::copy_nonoverlapping(
                            part.longitude.as_ptr(),
                            longitude_slot.as_mut_ptr().cast::<f64>(),
                            part.longitude.len(),
                        );
                    }
                });
    ...
    unsafe {
        cols.city_data.set_len(city_total);
        cols.city_offsets.set_len(1 + rows_total);
        cols.latitude.set_len(rows_total);
        cols.longitude.set_len(rows_total);
    }
    // Validity bitmaps are rows/8 bytes; the serial bit-shift join
    // is noise next to the scattered column copies above.
    let mut rows_done = 0usize;
    for part in &parts {
        append_bitmap(&mut cols.city_valid, rows_done, &part.city_valid, part.rows);
        append_bitmap(&mut cols.latitude_valid, rows_done, &part.latitude_valid, part.rows);
        append_bitmap(&mut cols.longitude_valid, rows_done, &part.longitude_valid, part.rows);
        rows_done += part.rows;
    }
...
fn index_csv_geo_text_stats_dispatch(data: &[u8], seed: u64, start: usize, sink: &mut CsvGeoTextStatsSink) {
    ...
    {
        // SAFETY: the required target features were just detected.
        unsafe { avx512::index_csv_geo_text_stats_compress(data, seed, start, sink) };
        return;
    }
    #[cfg(target_arch = "x86_64")]
    if std::arch::is_x86_feature_detected!("avx512f")
...
mod avx512 {
    ...
    /// [`index_csv_geo_text_stats`] twin for AVX-512 VBMI2 + BMI2 hosts: block structural
    /// positions arrive via one `vpcompressb` over a byte iota and terminator
    /// flags via `pext`, replacing the serial trailing-zero/clear-bit walk
    /// with a counted loop of independent loads. Scans 64-byte blocks from
    /// `start` (block-aligned) onward, until end of data or until the
    /// sink completes its record range.
    #[target_feature(enable = "avx512f", enable = "avx512bw", enable = "avx512vl", enable = "avx512vbmi2", enable = "bmi2", enable = "pclmulqdq")]
    pub(crate) fn index_csv_geo_text_stats_compress(data: &[u8], seed: u64, start: usize, sink: &mut super::CsvGeoTextStatsSink) {
        let mut carries = [seed];
        let mut offset = start;
        let iota = byte_iota();
        while offset + 64 <= data.len() {
